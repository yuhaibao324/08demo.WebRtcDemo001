...
<html>
	<head>
		...
		<style type="text/css">
			*{margin: 0; padding: 0; overflow-y: hidden;}
			body {background-color: rgb(ff, ff, ff);}
	...
	<body ondblclick="fullScreen()">
	   ...
	   2=${request.getparameter("username")}
		<hr>
	   <%
		   out.println("Your UserName: " + 	  		request.getParameter("username") );
	   %>
	   <hr>
	   ${pageContext.request.queryString}
		...
		<script type="text/javascript">
			...
			function openChannel() {
				...
				socket = new WebSocket("ws://192.168.1.100:8080/WebRtcDemo/acgist.video/${requestScope.uid}");
			...
			function noticeMsg() {
				if (!initiator) {
					setNotice("让别人加入（注意事项查看源码）: http://192.168.199.152:8080/WebRtcDemo/msg?oid=${requestScope.uid }");
				} else {
					setNotice("初始化...");
				}
			}
			...
			function createPeerConnection() {
				var server = {"iceServers" : [{"url" : "stun:stun.ideasip.com"}]};
